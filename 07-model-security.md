...
Membership inference attacks answer a narrower question than extraction: was this specific record part of the training set? An attacker who can query the model and observe how confidently it responds to a candidate record — a model is typically more confident, and its loss is typically lower, on data it was trained on than on unseen data — can infer training-set membership above chance, even without ever seeing the training data directly. Formally, if the attack is treated as a binary classifier guessing "member" or "non-member" for a candidate record, its success is measured the same way any detector's is: membership-inference advantage `Adv = TPR - FPR`, the gap between how often it correctly flags a true training-set member (true positive rate) and how often it incorrectly flags a non-member (false positive rate). An advantage of 0 means the attack does no better than a coin flip regardless of how confident it sounds; an advantage approaching 1 means it reliably distinguishes members from non-members. It's the same TPR/FPR shape used to score any detector's real-world performance, not a privacy-specific metric invented from scratch. (Yeom, Giacomelli, Fredrikson, and Jha, "Privacy Risk in Machine Learning: Analyzing the Connection to Overfitting," IEEE CSF 2018 / arXiv:1709.01604.) This matters most where training-set membership itself is sensitive: a model trained on a hospital's patient records, a fraud model trained on a bank's flagged transactions, or an HR model trained on employee performance reviews. Confirming that a named individual's record was in the training set can be a privacy violation on its own, independent of any other data recovered.
...
The best-known formalization of "small, targeted perturbation" is the Fast Gradient Sign Method (FGSM). Given an input `x`, a model with parameters `theta`, a loss function `J`, and the input's true label `y`, FGSM builds the adversarial example as:

```
x' = x + epsilon * sign( gradient_x J(theta, x, y) )
```

`epsilon` is what caps the perturbation — it bounds how far `x'` is allowed to drift from `x` in any single input dimension (`||x' - x||∞ <= epsilon`), which is precisely the "imperceptible" constraint the stop-sign example above depends on. Taking a single step in the direction of the loss gradient's sign, rather than iterating, is what makes FGSM a cheap, one-backward-pass attack — the tradeoff is that it's also weaker than iterative descendants like PGD (Projected Gradient Descent), which repeat that same bounded step to search for a stronger perturbation at higher compute cost. (Goodfellow, Shlens, and Szegedy, "Explaining and Harnessing Adversarial Examples," ICLR 2015 / arXiv:1412.6572 — see Further Reading.)
...
- **Yeom, S., Giacomelli, I., Fredrikson, M., Jha, S., "Privacy Risk in Machine Learning: Analyzing the Connection to Overfitting"** (2017/2018, IEEE CSF). `https://arxiv.org/abs/1709.01604` — formalizes membership-inference success as an adversary's advantage, `Adv = TPR - FPR`, and connects it to model overfitting.
